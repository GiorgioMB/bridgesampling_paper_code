...
for (k in 10:30) {
  ...
  # Fit model
  set.seed(k)
  samples <- stan_glm(y ~ X, data=df, mean_PPD=FALSE, prior=hs(), seed=1, cores=parallel::detectCores(), diagnostic_file=file.path(tempdir(), "df2.csv"), chains = 40)
  results_mine <- bridge_sampler(samples, method = "normal", num_splits = 6, total_perms = 100, return_always = TRUE, pareto_smoothing_all = TRUE, cores = parallel::detectCores())[[1]]
