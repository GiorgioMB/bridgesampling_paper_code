...
results <- readRDS("/scratch/work/micaleg1/bridge_sampling/glm_storing_everything_smaller_model.rds")
for (k in 10:104) {
  ...
  results_mine <- bridge_sampler(samples, method = "normal", num_splits = 6, total_perms = 100, return_always = TRUE, cores = parallel::detectCores())[[1]]
  ...
  saveRDS(results, "/scratch/work/micaleg1/bridge_sampling/glm_storing_everything_smaller_model.rds")
