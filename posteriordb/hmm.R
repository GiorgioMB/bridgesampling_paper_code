...
pd <- pdb_github()
posterior_names(pd)
po <- posterior("hmm_example-hmm_example", pdb = pd)
sc <- stan_code(po)
data <- get_data(po)
model <- stan_model(model_code = sc)
write_stan_file(sc, dir = getwd(), basename = "hmm.stan")
model_cmdstanr <- cmdstan_model("hmm.stan", force_recompile = TRUE)
...
write.csv(results, file = "hmm_pathfinder.csv", row.names = FALSE)
...
write.csv(results, file = "hmm_pathfinder_no_cov.csv", row.names = FALSE)
...
write.csv(results_bruteforce, file = "hmm_pathfinder_bruteforce.csv", row.names = FALSE)
