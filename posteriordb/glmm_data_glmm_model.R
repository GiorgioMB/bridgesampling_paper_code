...
set.seed(123)
pd <- pdb_github()
po <- posterior("GLMM_data-GLMM1_model", pdb = pd)
sc <- stan_code(po)
data <- get_data(po)
model <- stan_model(model_code = sc)
write_stan_file(sc, dir = getwd(), basename = "GLMM_data-GLMM1_model.stan")
model_cmdstanr <- cmdstan_model("GLMM_data-GLMM1_model.stan", force_recompile = TRUE)
...
write.csv(results, file = "glmm_data_model_pathfinder.csv", row.names = FALSE)
...
write.csv(results, file = "glmm_data_model_pathfinder_no_cov.csv", row.names = FALSE)
...
write.csv(results, file = "glmm_data_model_pathfinder_smoothed.csv", row.names = FALSE)
...
write.csv(results, file = "glmm_data_model_pathfinder_smoothed_no_cov.csv", row.names = FALSE)
...
write.csv(results_bruteforce, file = "glmm_data_model_pathfinder_bruteforce.csv", row.names = FALSE)
...
write.csv(results_bruteforce, file = "glmm_data_model_pathfinder_bruteforce_no_cov.csv", row.names = FALSE)
...
write.csv(results_bruteforce, file = "glmm_data_model_pathfinder_bruteforce_smoothed.csv", row.names = FALSE)
...
write.csv(results_bruteforce, file = "glmm_data_model_pathfinder_bruteforce_smoothed_no_cov.csv", row.names = FALSE)
