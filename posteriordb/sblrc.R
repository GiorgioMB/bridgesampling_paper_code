...
res <- bridge_sampler(fit_stan, method = "student_t", num_splits = 6, total_perms = 300, seed = 1, return_always = TRUE, verbose = TRUE, cores = parallel::detectCores())
split_1 <- res[[1]]
split_2 <- res[[2]]
res <- split_1
numi_split <- I(list(lapply(res, function(x) x$numi)))
...
for (i in 1:100) {
  ...
  fit_stan <- model_cmdstanr$sample(data = data,
                                    ...
                                    init = init_val,
                                    seed = i)
  res <- bridge_sampler(fit_stan, method = "student_t", seed = i, return_always = TRUE, verbose = TRUE, cores = parallel::detectCores())
  numi[[i]] <- res$numi
  deni[[i]] <- res$deni
