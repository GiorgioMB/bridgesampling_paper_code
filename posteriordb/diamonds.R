...
set.seed(123)
pd <- pdb_github()
po <- posterior("diamonds-diamonds", pdb = pd)
sc <- stan_code(po)
data <- get_data(po)
model <- stan_model(model_code = sc)
write_stan_file(sc, dir = getwd(), basename = "diamonds.stan")
model_cmdstanr <- cmdstan_model("diamonds.stan", force_recompile = TRUE)
...
write.csv(results, file = "diamonds_pathfinder.csv", row.names = FALSE)
...
write.csv(results, file = "diamonds_pathfinder_no_cov.csv", row.names = FALSE)
...
write.csv(results_bruteforce, file = "diamonds_pathfinder_bruteforce.csv", row.names = FALSE)
...
pareto_k_deni_20_brute <- extract_khat(deni, n_draws = 20)[[1]]
results_bruteforce <- cbind(results_bruteforce, pareto_k_numi_10_brute, pareto_k_deni_10_brute, pareto_k_numi_20_brute, pareto_k_deni_20_brute)
write.csv(results_bruteforce, file = "diamonds_pathfinder_bruteforce_no_cov.csv", row.names = FALSE)
